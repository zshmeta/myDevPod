...
$DEVPOD_VERSION = 'by zshmeta';
$DEVPOD_DATE = '09 - 04 - 2023';
$DEVPOD_API_PAGE = 'devpod-api/status.json';
...
$LOG_DIR	= dirname(dirname($CONF_DIR)) . DIRECTORY_SEPARATOR . 'log' . DIRECTORY_SEPARATOR . 'devpod';
		...
		switch($class) {
			...
			case 'Redis':
				...
				if ($REDIS_ROOT_PASSWORD == '') {
					$_LOADED_LIBS[$class] = \devpod\Redis::getInstance($GLOBALS['REDIS_HOST_NAME']);
				} else {
					$_LOADED_LIBS[$class] = \devpod\Redis::getInstance($GLOBALS['REDIS_HOST_NAME'], array(
			...
			case 'Memcd':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \devpod\Memcd::getInstance($GLOBALS['MEMCD_HOST_NAME']);
				break;

			case 'Mongo':
				loadFile($class, $cnt_dir);
				$_LOADED_LIBS[$class] = \devpod\Mongo::getInstance($GLOBALS['MONGO_HOST_NAME']);
				break;
