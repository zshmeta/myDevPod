			...
			function checkDns(vhost) {
				...
				xhttp.onreadystatechange = function(e) {
					...
					if (this.readyState == 4 && (this.status == 200 || this.status == 426)) {
						...
						//el_href.innerHTML = '(<a target="_blank" href="'+proto+'//localhost/devpod-project/'+name+'">ext</a>) <a target="_blank" href="'+proto+'//'+name+port+'">'+name+port+'</a>' + el_href.innerHTML;
						el_href.innerHTML = '<a target="_blank" href="'+proto+'//'+name+port+'">'+name+port+'</a>';
					} else {
						//console.log(vhost);
					}
				}
				xhttp.open('POST', proto+'//'+name+port+'/devpod-api/status.json', true);
				xhttp.send();
