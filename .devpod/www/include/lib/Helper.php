...
class Helper
{
	...
	{
		$dvl_password = loadClass('Helper')->getEnv('DEVPOD_UI_PASSWORD');

		if ($username == 'devpod' && $password == $dvl_password) {
			$_SESSION['auth'] = 1;
			return true;
		}
		return false;
	...
	public function isLoginProtected()
	{
		...
		if (loadClass('Helper')->getEnv('DEVPOD_UI_PROTECT') != 1) {
			return false;
		}
		return true;
