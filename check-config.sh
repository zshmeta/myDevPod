...
DEVPOD_PATH="$( get_env_value "DEVPOD_PATH" )"
if [ ! -d "${DEVPOD_PATH}" ]; then
	log_err "Variable 'DEVPOD_PATH' directory does not exist: ${DEVPOD_PATH}"
	RET_CODE=$(( RET_CODE + 1))
	WRONG_ENV_FILES_VALUES=1
else
	log_debug "Variable 'DEVPOD_PATH' directory exists: ${DEVPOD_PATH}"
fi

DEVPOD_PATH_PERM="$( file_get_perm "${DEVPOD_PATH}" )"
if [ "${DEVPOD_PATH_PERM}" != "0755" ] && [ "${DEVPOD_PATH_PERM}" != "0775" ] && [ "${DEVPOD_PATH_PERM}" != "0777" ]; then
	log_err "Variable 'DEVPOD_PATH' directory must be 0755, 0775 or 0777. Has: ${DEVPOD_PATH_PERM}"
	RET_CODE=$(( RET_CODE + 1))
	WRONG_ENV_FILES_VALUES=1
else
	log_debug "Variable 'DEVPOD_PATH' directory has correct permissions: ${DEVPOD_PATH_PERM}"
fi

DEVPOD_PATH_PERM="$( file_get_uid "${DEVPOD_PATH}" )"
if [ "${DEVPOD_PATH_PERM}" != "${MY_UID}" ]; then
	log_err "Variable 'DEVPOD_PATH' directory uid must be ${MY_UID}. Has: ${DEVPOD_PATH_PERM}"
	RET_CODE=$(( RET_CODE + 1))
	WRONG_ENV_FILES_VALUES=1
else
	log_debug "Variable 'DEVPOD_PATH' diretory has correct uid: ${DEVPOD_PATH_PERM}"
fi

DEVPOD_PATH_PERM="$( file_get_gid "${DEVPOD_PATH}" )"
if [ "${DEVPOD_PATH_PERM}" != "${MY_GID}" ]; then
	log_err "Variable 'DEVPOD_PATH' directory gid must be ${MY_GID}. Has: ${DEVPOD_PATH_PERM}"
	RET_CODE=$(( RET_CODE + 1))
	WRONG_ENV_FILES_VALUES=1
else
	log_debug "Variable 'DEVPOD_PATH' diretory has correct gid: ${DEVPOD_PATH_PERM}"
fi
...
#--------------------------------------------------------------------------------------------------
print_head_1 "Checking required DEVPOD core directories exist"
...
#--------------------------------------------------------------------------------------------------
# DEVPOD Directory permissions
#--------------------------------------------------------------------------------------------------
print_head_1 "Checking devpod core directory permissions"
...
DEVPOD_DIR_PERM_WRONG=0
for search_dir in "${DEVPOD_DIRS[@]}"; do
	while read -r my_dir; do
		...
		if [ "${PERM}" != "0755" ] && [ "${PERM}" != "0775" ] && [ "${PERM}" != "0777" ]; then
			...
			RET_CODE=$(( RET_CODE + 1))
			DEVPOD_DIR_PERM_WRONG=1
		else
			log_debug "Directory '${my_dir}' has correct permissions: ${PERM}"
		fi
	done < <(find "${search_dir}" -type d)
done
if [ "${DEVPOD_DIR_PERM_WRONG}" = "0" ]; then
	log_ok "All devpod directories have correct permissions"
fi
...
if [ "${DEVPOD_DIR_PERM_WRONG}" = "0" ]; then
	log_ok "All devpod directories have correct uid"
fi
...
if [ "${DEVPOD_DIR_PERM_WRONG}" = "0" ]; then
	log_ok "All devpod directories have correct gid"
fi


#--------------------------------------------------------------------------------------------------
# DEVPOD File permissions
#--------------------------------------------------------------------------------------------------
print_head_1 "Checking devpod core file permissions"
...
DEVPOD_DIR_PERM_WRONG=0
for search_file in "${DEVPOD_DIRS[@]}"; do
	while read -r my_file; do
		PERM="$( file_get_perm "${my_file}" )"
		# Private CA file
		if [ "${my_file}" = "ca/devpod-ca.key" ]; then
			if [ "${PERM}" != "0600" ]; then
...
if [ "${DEVPOD_DIR_PERM_WRONG}" = "0" ]; then
	log_ok "All devpod files have correct permissions"
fi
...
if [ "${DEVPOD_DIR_PERM_WRONG}" = "0" ]; then
	log_ok "All devpod files have correct uid"
fi
...
if [ "${DEVPOD_DIR_PERM_WRONG}" = "0" ]; then
	log_ok "All devpod files have correct gid"
fi
...
if [ "${RET_CODE}" -gt "0" ]; then
	...
	log_err "DEVPOD might not work properly"
